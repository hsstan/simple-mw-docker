...
then
  ...
  then
    ...
    echo "> Setting up SQlite database using initialization script"
  ...
  { \
    echo "#!/bin/sh" ; \
    echo "cd ${WIKI_DIR}" ; \
    echo "php maintenance/run.php sqlite --vacuum >> ${DATA_DIR}/log/mw_update.log 2>&1" ; \
...
then
  ...
  then
    ...
    php maintenance/run.php install --dbuser $DATABASE_NAME --dbpass $DATABASE_NAME --dbname $DATABASE_NAME --pass MEDIAWIKI_ADMIN_PASSWORD $DATABASE_NAME Admin
...
php maintenance/run.php update --quick > $LOG_DIR/mw_update.log
if [ $? -gt 0 ]
then
...
php maintenance/run.php createAndPromote --bureaucrat --sysop --force Admin ${MEDIAWIKI_ADMIN_PASSWORD}